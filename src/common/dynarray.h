...
public:
  ...
  dynarray(std::size_t sz) { init(sz); }
  ~dynarray(void) { clear(); }
  ...
  template<typename T = uint8_t> constexpr T* dataAtOffset (std::size_t offset) const
  {
  ...
  segment<T> segment(std::size_t offset, std::size_t length)
  {
